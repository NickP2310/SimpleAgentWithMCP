"""Main entry point for LLM-powered AI Agent.

This version uses OpenAI's API for real AI reasoning.
"""
import sys
import os
...
def print_banner():
    ...
    print("=" * 60)
    print("  AI Agent MCP Learning Project - LLM Powered")
    print("  Using OpenAI API for real AI reasoning")
    print("=" * 60)
    print()


def print_trace(agent: LLMAgent):
...
def get_api_key() -> str:
    ...
    if not api_key:
        print("❌ No API key provided. Cannot use LLM agent.")
        sys.exit(1)
...
def run_prompt(prompt: str, api_key: str):
    """Execute a single prompt with LLM agent."""
...
def run_interactive(api_key: str):
    """Run interactive mode with LLM agent."""
    print("🎮 Interactive Mode (LLM-Powered)")
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage:")
        print("  python main_llm.py interactive              - Start interactive mode")
        print("  python main_llm.py prompt \"<your prompt>\"   - Execute single prompt")
        print()
        print("Examples:")
        print("  python main_llm.py interactive")
        print("  python main_llm.py prompt \"What is Python and how do I learn it?\"")
        print("  python main_llm.py prompt \"Calculate 42 * 17 and explain the result\"")
        print()
